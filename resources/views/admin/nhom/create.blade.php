...
@section('content')
    <div class="p-4 h-full overflow-scroll">
        ...
        <div>
            <form method="POST" action="{{ route('nhom.store') }}">
                @csrf
                        ...
                        <select id="hocKy" name="namHoc"
                            ...
                            @for ($year = 2010; $year <= date('Y'); $year++)
                                @php
                                     $Preyear = $year - 1;
                                @endphp
                                <option value='{{ "$Preyear - $year" }}'>{{ "$Preyear - $year" }}</option>
                            @endfor
                        </select>
                    </div>
                    <div>
                ...
                <div class="mt-4">
                    ...
                    <input type="text" class="w-full border-2 p-1 rounded-[6px] outline-none" name="tenNhom">
                </div>
                <div class="mt-4">
                    <div>
                        <label class="text-Mplanet text-sm font-medium" for="tenMon">Sĩ số</label>
                    </div>
                    <input type="text" class="w-full border-2 p-1 rounded-[6px] outline-none" name="siSo">
                </div>
                <div class="mt-4">
