...
@section('content')
    <div class="p-4 h-full overflow-scroll">
        <h4 class="my-4 text-xl text-Mgray font-normal">{{ $nhom->tennhom }}</h4>
        <div class="flex justify-between items-center">
            <a href="{{ route('nhom.adduser', ['id' => $nhom->id]) }}" class="btn-primary">
                <i class="fa-solid fa-plus mr-1"></i>THÊM NGƯỜI DÙNG
            </a>
                        ...
                        @foreach ($dsUser as $user)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <form method="post" action="{{ route('chitietnhom.destroy',['id' => $nhom->id]) }}">
                                        @method('delete')
                                        @csrf
                                        <input type="hidden" value="{{ $user->id }}" name='id_user'>
                                        <button data-id="{{ $user->id }}" type="submit" class="btn-danger btn_delete"><i class="fa-solid fa-xmark"></i></button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
