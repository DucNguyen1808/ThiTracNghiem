...
@section('content')
    <div class="p-4 h-full overflow-scroll">
        <h4 class="my-4 text-xl text-Mgray font-normal">Đề thi</h4>
        <div class="flex justify-between items-center">
            <a href="{{ route('dethi.create') }}" class="btn-primary">
                <i class="fa-solid fa-plus mr-1"></i>THÊM ĐỀ THI
            </a>
            <div class="w-1/2 flex justify-end items-center">
                <a href="{{ route('dethi.index') }}"><i class="mr-4 btn-primary fa-solid fa-rotate-right"></i></a>
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3">
                                Thời Gian Bắt Đầu
                                {!! sortable('tgmode', $sort) !!}
                            </th>
                            <th scope="col" class="px-6 py-3">
                                Thời Gian Kết Thúc
                                {!! sortable('tgketthuc', $sort) !!}
                            </th>
                            <th scope="col" class="px-6 py-3">
                                Môn học
                            </th>
                    ...
                    <tbody>
                        @foreach ($dsDeThi as $dethi)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4">
                                    <button data-id="{{ $dethi->id }}" type="submit" class="btn-danger btn_delete"
                                        href=""><i class="fa-solid fa-trash-can"></i></button>
                                    <a class="btn-warning" href="{{ route('dethi.edit', ['dethi' => $dethi->id]) }}"><i
                                            class="fa-regular fa-pen-to-square"></i></a>
                                    <a class="btn-primary" href="{{ route('dethi.show', ['dethi' => $dethi->id]) }}"><i class="fa-solid fa-gear"></i></a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    <script>
        ...
        deleteBtns.forEach(btn => {
            btn.onclick = (e) => {
                ...
                deThiId = btn.dataset.id
            }
        })
        btnDeleteNhom.onclick = function() {
            deleteForm.action = `/admin/dethi/${deThiId}`
            deleteForm.submit();
        }
    </script>
@endsection
